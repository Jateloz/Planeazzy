...
<main class="auth-wrap">
  <div class="auth-card slide-up">
    <div style="text-align:center;margin-bottom:24px">
      <div style="width:56px;height:56px;border-radius:14px;background:var(--teal-10);color:var(--teal);display:flex;align-items:center;justify-content:center;margin:0 auto 14px;font-size:24px">
        <i class="fa-solid fa-envelope-circle-check"></i>
      </div>
      <h2 style="font-size:1.375rem;font-weight:900;color:var(--slate-900);margin-bottom:8px;letter-spacing:-.03em"
          data-en="Verify your provider account" data-sw="Thibitisha akaunti yako ya mtoa huduma">Verify your provider account</h2>
      <p style="font-size:.875rem;color:var(--slate-500);line-height:1.65">
        ...
        <span data-en="Valid for" data-sw="Inafaa kwa">Valid for</span> <?= OTP_EXPIRY_MINUTES ?> <span data-en="minutes." data-sw="dakika.">minutes.</span>
      </p>
    </div>
    ...
    <button id="verifyBtn" class="btn btn-primary btn-full btn-lg" disabled onclick="doVerify()" style="background:var(--teal)">
      <i class="fa-solid fa-check-circle"></i>
      <span data-en="Verify Account" data-sw="Thibitisha Akaunti">Verify Account</span>
    </button>

    <div style="margin-top:14px;text-align:center;font-size:.875rem">
      <span style="color:var(--slate-500)" data-en="Didn't receive a code?" data-sw="Hukupokea nambari?">Didn't receive a code?</span>
      <button onclick="doResend()" style="background:none;border:none;cursor:pointer;color:var(--teal);font-weight:700;font-family:inherit;font-size:.875rem;text-decoration:underline"
              data-en="Resend" data-sw="Tuma tena">Resend</button>
    </div>
  </div>
</main>
...
async function doVerify() {
  const r = await post('/api/provider/verify-otp.php', {
    ...
    provider_id: parseInt(sessionStorage.getItem('pz_prov_id') || 0),
    otp: OTP.value('#otpGrid')
  }, 'verifyBtn', 'alertBox');
  if (!r) return;
  if (r.success) {
    UI.alert('ok', document.documentElement.lang === 'sw' ? 'Akaunti imethibitishwa!' : 'Account verified! Redirecting…', 'alertBox');
...
async function doResend() {
  ...
  if (r?.success) UI.alert('ok', document.documentElement.lang === 'sw' ? 'Nambari mpya imetumwa!' : 'New code sent!', 'alertBox');
}
</script>
